        ...
        static void Main(string[] args)
        {
            ...
            foreach (var test in testCases)
            {
                ...
                try
                {
                    ...
                    {
                        Console.WriteLine($"❌ 识别失败: 未找到条形码");

                        // 尝试更激进的识别参数
                        Console.WriteLine("\n尝试激进识别参数...");
                        var aggressiveResult = BarcodeReader.Read(test.File,
                            BarcodeReader.Encoding.UTF8,
                            BarcodeReader.BarcodeRotation.Any,
                            true,  // TryHarder
                            true   // TryInverted
                        );
